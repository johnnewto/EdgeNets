...
class SuffleNetv2Segmentation(nn.Module):
    '''
    This class defines the Shufflenet architecture for the Semantic Segmenation
    '''
...
def shufflenetv2_seg(args, classes, dataset='city', scales=[2.0, 1.5, 1.0, 0.5, 0.1]):
    weights = args.weights
    model = SuffleNetv2Segmentation(args, classes=classes, dataset=dataset, scales=scales)
    if weights:
        import os
...
if __name__ == "__main__":
    ...
    for scale in segmentation.dicenet_scales:
        ...
        if args.dataset == 'pascal':
            for size in [256, 384]:
                model = shufflenetv2_seg(args, classes=21)
